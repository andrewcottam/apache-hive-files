...
DROP TABLE species.species_unique_2014_2_index; 
CREATE TABLE species.species_unique_2014_2_index STORED AS RCFILE AS SELECT id_no, cell, fully_covered, cell_intersect, IF(cell_intersect IS NULL, geotools_cell_area(cell, 1.0, "EPSG:4326", "EPSG:54009"), geotools_area(ST_Transform("EPSG:4326", "EPSG:54009", cell_intersect))) area FROM (SELECT id_no, c.cell, c.fully_covered, IF(fully_covered, NULL, ST_CellIntersection(1.0, c.cell, geom)) cell_intersect FROM species.species_unique_2014_2 LATERAL VIEW esri_cells_udtf(1.0, geom) c) sub; 
